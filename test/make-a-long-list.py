    ...
    while switchnum < maxswitch:
        for octet4 in [0,32,64,96,128,160,192,224]:
            if switchnum < maxswitch:
                ...
                ipstart = octet4 + 5
                ipend = ipstart + 23
                cameraiprange = '192.168.' + str(octet3) + '.' + str(ipstart) + '-' + str(ipend)
                gw = '192.168.' + str(octet3) + '.' + str(octet4 + 1)
                address = '192.168.' + str(octet3) + '.' + str(octet4) + '/27'
                description = 'Access switch {3}-{4}-ws-sc-acc-sw{2} camera subnet'.format(octet3,octet4,switchnum,az,aznum,address,gw)
